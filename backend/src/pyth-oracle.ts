...
export async function rollingChainLoop(stream: PythStream): Promise<void> {
  ...
  async function createMarket(
    ...
    try {
      ...
      let absoluteDeadlineMs: number
      try {
        const freshBlock = await getAleoBlockHeight()
        const blocksAhead = Number(deadline - freshBlock)
        const msUntilDeadline = Math.max(0, blocksAhead * SECS_PER_BLOCK * 1000)
        absoluteDeadlineMs = Date.now() + msUntilDeadline
      } catch (err) {
        console.warn(`[chain] ${symbol} fresh-block fetch failed, using creation-time estimate:`, err)
        // Fallback: use the pre-broadcast currentBlock (slightly stale, off
        // by the tx-confirm duration — still better than nothing)
        const blocksAhead = Number(deadline - currentBlock)
        absoluteDeadlineMs = Date.now() + blocksAhead * SECS_PER_BLOCK * 1000
      }
