...
export function Landing() {
  ...
  return (
    <div className="min-h-screen bg-surface-950 relative overflow-hidden">
      ...
      <section className="relative min-h-[70vh] flex items-center justify-center pt-16 pb-8 z-10">
          ...
          <div className="grid lg:grid-cols-2 gap-6 items-center">
            ...
            >
              <div className="bg-surface-900/80 backdrop-blur-xl rounded-2xl border border-brand-500/20 overflow-hidden shadow-2xl">
                ...
                <div className="bg-surface-800/50 px-4 py-3 flex items-center gap-2 border-b border-brand-500/10">
                  ...
                  <span className="text-xs text-surface-400 font-mono ml-2">veiled_markets.aleo</span>
                </div>

                {/* Terminal Content */}
                <div className="p-6 font-mono text-sm space-y-3">
                  <TerminalLine delay={0.2} prompt="$" text="aleo connect --wallet puzzle" />
                  <TerminalLine delay={0.4} text="✓ Wallet connected: aleo1..." color="text-yes-400" />
                  <TerminalLine delay={0.6} prompt="$" text="veiled bet --market 0x4f2a --amount 100 --side YES" />
                  <TerminalLine delay={0.8} text="⚡ Generating zero-knowledge proof..." color="text-accent-400" />
                  <TerminalLine delay={1.0} text="✓ Bet placed privately" color="text-yes-400" />
                  <TerminalLine delay={1.2} text="🔒 Your position: ENCRYPTED" color="text-brand-400" />
                  <TerminalLine delay={1.4} text="📊 Market odds updated: 67% YES" color="text-surface-400" />
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
            ...
            >
              ...
              <motion.div
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-brand-500/10 border border-brand-500/20 mb-3"
              >
              ...
              <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-3 leading-tight">
              ...
              <p className="text-base sm:text-lg text-surface-400 mb-4 leading-relaxed">
              ...
              {/* Supported Wallets */}
              <div className="mt-3 flex items-center gap-3 text-surface-500 text-sm">
                <span>Supports:</span>
                <span className="text-surface-400">Puzzle</span>
                <span className="text-surface-600">•</span>
                <span className="text-surface-400">Leo</span>
                <span className="text-surface-600">•</span>
                <span className="text-surface-400">Demo Mode</span>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="relative py-8 border-t border-surface-800/30 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-6">
            {[
              { num: '01', title: 'Connect', desc: 'Link your Aleo wallet' },
              { num: '02', title: 'Browse', desc: 'Explore prediction markets' },
              { num: '03', title: 'Bet', desc: 'Place encrypted bets with ZK proofs' },
              { num: '04', title: 'Claim', desc: 'Privately collect your winnings' },
            ].map((step, i) => (
              <motion.div
                key={step.num}
                initial={{ opacity: 0, x: -20 }}
              ...
              >
                <div className="text-5xl font-bold text-brand-500/20 group-hover:text-brand-500/40 transition-colors font-mono">
                  {step.num}
                </div>
                <div className="flex-1 border-l-2 border-brand-500/20 pl-6 py-3">
      ...
      {/* Footer */}
      <footer className="relative border-t border-surface-800/30 py-6 z-10">
...
function TerminalLine({
  ...
  return (
    ...
    >
      {prompt && <span className="text-brand-400">{prompt}</span>}
...
function StatCard({
  ...
  return (
    <div className="bg-surface-800/30 backdrop-blur-sm rounded-xl p-4 border border-brand-500/10">
...
function FeaturePill({
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-surface-800/50 border border-surface-700/50 text-surface-300 text-sm">
      <div className="w-4 h-4 text-brand-400">
        {icon}
      </div>
      <span>{text}</span>
    </div>
...
function BentoCard({
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      transition={{ delay }}
      className="group relative bg-surface-900/50 backdrop-blur-sm rounded-xl p-5 border border-surface-800/50 hover:border-brand-500/30 transition-all duration-300"
    >
      ...
      <div className="relative">
        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-brand-500/20 to-accent-500/20 flex items-center justify-center mb-3 text-brand-400 group-hover:scale-110 transition-transform">
          {icon}
        </div>
